  ...
  fetch(url, {
    ...
    .then((json) => {
      if (json.success) {
        ...
        Swal.fire("จัดส่งไม่สำเร็จ", "", "error");
      }
    });
}
  ...
  fetch(url, {
    ...
    .then((json) => {
      if (json.success) {
        ...
        Swal.fire("ย้อนกลับการจัดส่งไม่สำเร็จ", "", "error");
      }
    });
}
  ...
  fetch(url)
    ...
    .then((json) => {
      if (json.success) {
        ...
        document
          .getElementById("btnUpdateTrack")
          .setAttribute("onclick", "createTracking(" + deliveryId + ")");
        $("#addTracking").modal("show");
      } else {
        Swal.fire("เรียกข้อมูลไม่สำเร็จ", "", "error");
      }
    });
}
  ...
  fetch(url, {
    ...
    .then((json) => {
      if (json.success) {
        ...
        Swal.fire("อัปเดทเลขพัสดุไม่สำเร็จ", "", "error");
      }
    });
}
